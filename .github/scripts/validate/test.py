...
class TestValidation(unittest.TestCase):
    def test_valid_packages(self):
        for package in VALID_PACKAGES:
            self.assertEqual(
                len(validate_package(os.path.join(TEST_PACKAGES_DIR, package))), 0
            )
    ...
    def assertValidationError(self, errors, name: str):
        ...
        self.assertTrue(
            False,
            f"expected validation error: {name}, but it's not included in errors: {error_names}",
        )
    ...
    def test_missing_version_path(self):
        errors = validate_package(
            os.path.join(TEST_PACKAGES_DIR, "missing-version-path")
        )
        self.assertValidationError(errors, "invalid_version_path")

    def test_invalid_extra_file_in_versions(self):
        errors = validate_package(
            os.path.join(TEST_PACKAGES_DIR, "invalid-extra-file-in-versions")
        )
        self.assertValidationError(errors, "invalid_version_path")

    def test_invalid_package_name(self):
        errors = validate_package(
            os.path.join(TEST_PACKAGES_DIR, "invalid_package_name")
        )
        self.assertValidationError(errors, "invalid_package_name")

    def test_path_coherence_name(self):
        errors = validate_package(
            os.path.join(TEST_PACKAGES_DIR, "mismatch-package-name")
        )
    ...
    def test_manifest_missing_fields(self):
        errors = validate_package(
            os.path.join(TEST_PACKAGES_DIR, "manifest-missing-fields")
        )
        self.assertValidationError(errors, "missing_manifest_fields")

    def test_invalid_yaml_manifest(self):
        errors = validate_package(
            os.path.join(TEST_PACKAGES_DIR, "invalid-manifest-yaml")
        )
